...
extern uint8_t current_ntp_index ;
...
1: socket timeout
*/
extern uint8_t NTPSendSTT;

void get_gpsStatus_all(void *ptr, uint8_t *len);
void get_NTPServer(void *ptr, uint8_t *len);
void get_POWER_stt(void *ptr, uint8_t *len);

void get_NTPServer(void *ptr, uint8_t *len)
{
	*len = sprintf((char *)ptr, "NTP server: %s; IP:%d.%d.%d.%d, found %d", ntpServers[current_ntp_index],ntp_ip[0],ntp_ip[1],ntp_ip[2],ntp_ip[3],ntpServersFound);
}

void get_POWER_stt(void *ptr, uint8_t *len)
{
	*len = sprintf((char *)ptr, "Socket:%s,TZ %d,DNS %d", NTPSendSTT?"Timeout":"OK",timeZone,useDNS);
}
...
dataEntryType snmpData[] =
{
	...
	SNMPDTYPE_OCTET_STRING, 30, {"Slave console clock, NTP clock time client 2024"},
	NULL, NULL},
	#endif
	#ifdef SLAVE_WALL
	{8, {0x2b, 6, 1, 2, 1, 1, 1, 0},
	SNMPDTYPE_OCTET_STRING, 30, {"Slave wallclock, NTP clock time client 2024"},
	NULL, NULL},
	#endif

	#ifdef SLAVE_MATRIX
	{8, {0x2b, 6, 1, 2, 1, 1, 1, 0},
	SNMPDTYPE_OCTET_STRING, 30, {"Slave matrix clock, NTP clock time client 2024"},
	NULL, NULL},
	#endif
