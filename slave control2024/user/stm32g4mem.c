 ...
 /* Clear OPTVERR bit set on virgin samples */
  ...
  temp=0;
		...
		else 
    {
		...
		temp = 0;
		if((useDNS<0)&&(useDNS>1)) useDNS = 0;
		if((timeZone<0)&&(timeZone>12)) timeZone = 0;
		//temp = (0xFF <<24) |(0xFF <<16)|(timeZone <<8)|useDNS;
		temp = ((uint64_t)0xFFFF00 << 16) | ((uint64_t)timeZone << 8) | useDNS;
		
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, FLASH_USER_START_ADDR+32, temp) == HAL_OK)
    {
			#ifdef DebugEnable
			//printf("Saved timeZone+ DNS config\r\n");
			#endif
    }
		else 
    {
...
void stm32g474_FactoryLoad(void)
{
	if(myMem->keyword == 0x12345678)
	{
		...
		useDNS   = myMem->other[4];
		timeZone = myMem->other[5];
		//printf("load useDNS %d, timezone :%d, %d,%d,%d,%d,%d,%d\r\n",useDNS,timeZone,myMem->other[2],myMem->other[3],myMem->other[4],myMem->other[5],myMem->other[6],myMem->other[7]);
