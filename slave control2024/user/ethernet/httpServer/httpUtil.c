...
uint8_t * set_basic_config_setting(uint8_t * uri)
{
	...
	//boc tach IP
		...
		//tz
		if((param = get_http_param_value((char *)uri, "tz")))
		{
			inet_addr_((uint8_t*)param, &timeZone);
			//printf("time Zone: %d\r\n",timeZone);
		}
		//mutil ntp time servers
		if((param = get_http_param_value((char *)uri, "ntpserver")))
		{
			inet_addr_((uint8_t*)param, &useDNS);
			//printf("ntpserver: %d\r\n",useDNS);
		}
	return ip;
}
