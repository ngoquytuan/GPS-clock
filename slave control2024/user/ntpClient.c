...
#define SendAgainAfterSeconds 90
extern int8_t timeZone;
const uint8_t timeToReSendNtpMessage = SendAgainAfterSeconds;
#ifdef OverTheAir
...
0: sent ok
*/
uint8_t NTPSendSTT = 1;
uint8_t flag_to_switchNtpServer = 0;
...
#define sntp_port 100 //my sntp port, cho de gui ban tin ve
...
int8_t SNTP_run(void)
{
	...
	case SOCK_UDP:
		...
		{
			#ifdef NTP_CLIENT_DEBUG
				//printf("getSn_RX_RSR, size %d\r\n",size);	
			#endif
			if (size != 56) return 0;//wrong message NTP!!!
			...
			//const long gmtOffset_sec = 7 * 60 * 60;25200  // +7 timezone (Hanoi)
			//sec = 25200 + (ntpTimeServer_buf[40]<<24) + (ntpTimeServer_buf[41]<<16) + (ntpTimeServer_buf[42]<<8) + ntpTimeServer_buf[43] ;
			//UTC time + timeZone
			sec = (int)timeZone*60*60 + (ntpTimeServer_buf[40]<<24) + (ntpTimeServer_buf[41]<<16) + (ntpTimeServer_buf[42]<<8) + ntpTimeServer_buf[43] ;
			...
			if(((offset < 999) && (offset >= 0)) || ((offset > -999) && (offset < 0)))
			{
				...
				NTP_alreadySentTimeOut = timeToReSendNtpMessage;
				return 1;
			}
		...
		if(	NTP_alreadySent == 0)
			{
				...
				ret = sendto(SOCK_SNTP,ntpmessage,48,ntp_ip,123);
				counterOf10Second = 0;
				countOfNTPrequest++;
				...
				if     (ret == 48) 							NTPSendSTT = 0;//printf("Sent ok\r\n");
				else if(ret == SOCKERR_TIMEOUT)	NTPSendSTT = 1;//printf("Socket timeout\r\n");
				#ifdef NTP_CLIENT_DEBUG
				printf("NTP Sent to %d.%d.%d.%d, %d,flag_to_switchNtpServer: %d\r\n",ntp_ip[0],ntp_ip[1],ntp_ip[2],ntp_ip[3], ret,flag_to_switchNtpServer);
				#endif	
				return 0;
			}
...
//	break;
		case SOCK_CLOSED:
		{
			...
			while(retry_count < max_retries) {
				ret = socket(SOCK_SNTP, Sn_MR_UDP, sntp_port, 0x00);
				if(ret == SOCK_SNTP) {
					#ifdef NTP_CLIENT_DEBUG	
					printf(" Socket[%d] UDP Socket for SNTP client started at port [%d]\r\n", SOCK_SNTP, sntp_port);
					#endif
